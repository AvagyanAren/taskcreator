...
**Режим «Обычным языком»** — одно поле. Пишете задачу целиком, нажимаете
`Создать задачу` (или `Ctrl/Cmd + Enter`), смотрите превью, подтверждаете.
...
Если год не указан — берётся текущий; если такая дата уже прошла, используется
ближайшая будущая (следующий год).
