...
Во время записи показывается отдельная строка «Говорите…» с кнопкой **Стоп**.
Мобильные браузеры здесь ненадёжны: `onend` может не прийти вовсе, а `stop()`
— игнорироваться, поэтому состояние сбрасывается локально, распознавание
принудительно прерывается (`abort()`), и есть страховочный таймер на 20 секунд.
Интерфейс не может «залипнуть» в состоянии записи.
